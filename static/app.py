...
def init_db():
    ...
    if "email_usuario" not in cols_med:
        cursor.execute("ALTER TABLE medicamentos ADD COLUMN email_usuario TEXT NOT NULL DEFAULT ''")
...
def actualizar_email():
    ...
    try:
        ...
        cursor.execute('UPDATE usuarios SET email=? WHERE email=?', (email_nuevo, email_actual))
...
def actualizar_contrasena():
    ...
    try:
        ...
        cursor.execute('UPDATE usuarios SET password=? WHERE email=?', (pass_nueva_hash, email))
        conn.commit()
        conn.close()

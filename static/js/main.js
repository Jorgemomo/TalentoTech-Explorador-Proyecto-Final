...
function renderizarKanban(lista) {
  ...
  if (contadores.seguro   === 0) colSeguro.insertAdjacentHTML("beforeend",   '<p class="empty-msg">Sin medicamentos</p>');
  if (contadores.atencion === 0) colAtencion.insertAdjacentHTML("beforeend", '<p class="empty-msg">Sin medicamentos</p>');
  if (contadores.critico  === 0) colCritico.insertAdjacentHTML("beforeend",  '<p class="empty-msg">Sin medicamentos</p>');
...
async function cargarMedicamentos() {
  ...
  try {
    const res = await fetch(`${API_URL}/medicamentos?email=${encodeURIComponent(sesion.email)}`);
...
function aplicarFiltros() {
  document.getElementById("colSeguro").closest(".col-wrapper-seguro").style.display =
    filtrosActivos.seguro ? "" : "none";
  document.getElementById("colAtencion").closest(".col-wrapper-atencion").style.display =
    filtrosActivos.atencion ? "" : "none";
  document.getElementById("colCritico").closest(".col-wrapper-critico").style.display =
    filtrosActivos.critico ? "" : "none";
}
...
function mostrarToast(msg, tipo = "success") {
  ...
  setTimeout(() => { toast.className = "dash-toast"; }, 3500);
}
...
function toggleModoBorrado() {
  ...
  document.getElementById("bottom").classList.toggle("modo-borrado", modoBorrado);
  const btn = document.getElementById("btnBorrar");
  btn.textContent = modoBorrado ? "✖ Cancelar borrado" : "🗑️ Borrar medicamento";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  inputBuscar.addEventListener("keydown", e => {
    ...
    if (e.key === "Escape") { inputBuscar.value = ""; buscar(""); }
  });
  // Búsqueda en tiempo real
  inputBuscar.addEventListener("input", () => buscar(inputBuscar.value));

  // Botón configurar categorías → modal filtro
  document.getElementById("btnFiltro").addEventListener("click", abrirModalFiltro);
  document.getElementById("btnCerrarFiltro").addEventListener("click", cerrarModalFiltro);
  document.getElementById("btnAplicarFiltro").addEventListener("click", aplicarModalFiltro);
  // Cerrar modal al hacer clic fuera
  document.getElementById("modalFiltro").addEventListener("click", e => {
    if (e.target === document.getElementById("modalFiltro")) cerrarModalFiltro();
  });

  // Botón borrar medicamento → modo borrado
  document.getElementById("btnBorrar").addEventListener("click", toggleModoBorrado);
});
